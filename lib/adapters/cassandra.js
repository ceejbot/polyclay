...
CassandraAdapter.prototype.destroyMany = function(objlist, callback)
{
	...
	this.withKeyspace.then(function() { return self.connection.cql('DELETE from %s WHERE key IN (%s)', [self.family, keylist.join(', ')]); })
	.then(function(reply)
	{
		// TODO also delete attachments
		callback();
...
CassandraAdapter.prototype.removeAttachment = function(obj, name, callback)
{
	...
	this.withKeyspace.then(function() { return self.connection.cql('SELECT * from %s WHERE key = ?', [self.attachfamily, key]); })
	.then(function(reply)
	{
		callback(null, 'OK');
	})
