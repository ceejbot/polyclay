...
CassandraAdapter.prototype.configure = function(options, modelfunc)
{
	...
	this.link = this.connection.connect().then(function() { return self.assignKeyspace(); });
	this.withTables = this.link.then(function(keyspace) { return self.getTables(keyspace); });
};
...
CassandraAdapter.prototype.save = function(obj, json, callback)
{
	...
	.then(function() { return self.getAttachmentTable(); })
...
CassandraAdapter.prototype.get = function(key, callback)
{
	...
	this.link.then(function() { return self.connection.cql('SELECT * from %s WHERE key = ?', [self.family, key]); })
	.then(function(rows)
	{
...
CassandraAdapter.prototype.getBatch = function(keylist, callback)
{
	...
	this.link.then(function() { return self.connection.cql('SELECT * from %s WHERE key IN (%s)', [self.family, keylist]); })
	.then(function(rows)
	{
		rows.forEach(function(row)
		{
...
CassandraAdapter.prototype.all = function(callback)
{
	...
	this.link.then(function() { return self.connection.cql('SELECT * from %s', [self.family]); })
	.then(function(rows)
	{
		rows.forEach(function(row)
		{
...
CassandraAdapter.prototype.attachment = function(key, name, callback)
{
	...
	this.link.then(function() { return self.connection.cql('SELECT * from %s WHERE key = ?', [self.attachfamily, cassKey]); })
	.then(function(rows)
	{
		if (rows.length === 0)
...
CassandraAdapter.prototype.remove = function(obj, callback)
{
	...
	this.link.then(function() { return self.connection.cql('DELETE FROM %s WHERE KEY = ?', [self.family, obj.key]); })
	.then(function(reply)
	{
...
CassandraAdapter.prototype.destroyMany = function(objlist, callback)
{
	...
	this.link.then(function() { return self.connection.cql('DELETE from %s WHERE key IN (%s)', [self.family, keylist.join(', ')]); })
	.then(function(reply)
	{
		// TODO also delete attachments
		callback();
...
CassandraAdapter.prototype.removeAttachment = function(obj, name, callback)
{
	...
	this.link.then(function() { return self.connection.cql('SELECT * from %s WHERE key = ?', [self.attachfamily, key]); })
	.then(function(reply)
	{
		callback(null, 'OK');
	})
