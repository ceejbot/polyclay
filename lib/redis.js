...
RedisAdapter.prototype.get = function(key, Modelfunc, callback)
{
	...
	this.redis.hgetall(this.hashkey(key), function(err, payload)
	{
		...
		var object = RedisAdapter.inflate(Modelfunc, payload);
...
RedisAdapter.prototype.getBatch = function(keylist, Modelfunc, callback)
{
	...
	{
		if (err) return callback(err);
		var results = _.map(jsondocs, function(item) { return RedisAdapter.inflate(Modelfunc, item); });
...
{
	this.redis.hmset(this.hashkey(key), RedisAdapter.flatten(json), callback);
};
...
RedisAdapter.inflate = function(Modelfunc, payload)
{
	if (payload === null)
		return;
