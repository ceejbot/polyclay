...
// Model can accept requests over its socket or by local calls.
// Model can optionally use cradle's in-memory cache.
...
function persist(modelfunc)
{
	...
	{
		if (!modelfunc.prototype.__types)
			modelfunc.prototype.__types = {};
	...
	modelfunc.removeMany = function(ids, callback)
	{
		...
		{
			actionsList.push(makeRemoveFunc(ids[i].id, '*'));
		}
	...
	modelfunc.prototype._serializeAttachments = function()
	{
		...
		for (var i = 0; i < keys.length; i++)
		{
			...
			if (this.__attachments[name].__dirty)
			{
				...
				if (this.__attachments[name].body instanceof Buffer)
	...
	modelfunc.prototype.destroy = function(callback)
	{
		var self = this;
		if (!self._id)
			return callback(new Error('cannot delete object without an id'));
		if (self.deleted)
			return callback(new Error('object already deleted'));
	...
	modelfunc.prototype.handleAttachments = function(attachments)
	{
		...
		for (var i = 0; i < names.length; i++)
		{
			...
			this.__attachments[name].content_type = attach.content_type;
			this.__attachments[name].revpos = attach.revpos;
			this.__attachments[name].digest = attach.digest;
			this.__attachments[name].length = attach.length;
			this.__attachments[name].stub = attach.stub;
		}
	};
...
CouchAdapter.prototype.configure = function(options, credentials)
{
	// set up couch
