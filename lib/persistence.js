...
function persist(modelfunc, keyfield)
{
	...
	modelfunc.setStorage = function(options, Adapter)
	{
		modelfunc.adapter = new Adapter();
		modelfunc.adapter.configure(options, modelfunc);
	};
	...
	modelfunc.get = function(key, callback)
	{
		...
		modelfunc.adapter.get(key, function(err, object)
		{
			callback(err, object);
		});
	};
	...
	{
		var object, results = [];
		modelfunc.adapter.getBatch(keys, function(err, results)
		{
	...
	{
		var results = [], ids = [];
		modelfunc.adapter.all(function (err, ids, fullObjects)
		{
			if (err) return callback(err);
	...
	{
		modelfunc.adapter.provision(callback);
	};
	...
	modelfunc.constructMany = function(documents, callback)
	{
		...
		{
			return modelfunc.adapter.inflate(doc);
		});
	...
	modelfunc.prototype.save = function(callback)
	{
		...
		if (self.__new)
		{
			modelfunc.adapter.save(self, serialized, function(err, response)
			{
				if (err) return callback(err);
				self.clearDirty();
				self.__new = false;
	...
	modelfunc.prototype.saveAttachment = function(name, callback)
	{
		...
		modelfunc.adapter.saveAttachment(self, attach, function(err, response)
		{
			if (err) return callback(err);
